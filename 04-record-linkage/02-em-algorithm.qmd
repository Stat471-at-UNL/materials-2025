...
True match status is unobserved; use Expectation–Maximization (EM) algorithm:

1. **E-step:**  
   Estimate match memberships $P(M \mid \gamma)$ for each pair given current parameters.

2. **M-step:**  
   Update $m^{(i+1)} = P(\gamma \mid M), u^{(i+1)} = P(\gamma \mid U), P(M)$ based on expected match memberships.
...
Quantify uncertainty: e.g. trigger manual review for pairs that have posterior probabilities in $[T_L, T_U]$.
