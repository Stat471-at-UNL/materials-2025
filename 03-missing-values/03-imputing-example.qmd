...
\end{eqnarray*}
$$
i.e. only if $\rho > 0$, the expectation of $Y$ is affected by missing values.
...
dframe |> arrange(desc(type)) |>
  ...
  geom_smooth(method="lm", group=1, se=F,
              data = dframe, aes(colour="Full Sample")) +
  scale_colour_manual(values=c("grey40", "darkorange", "steelblue"))
```
...
```{r}
NHANES |> ggplot(aes(Age, fill = is.na(AlcoholDay))) + 
  geom_histogram(binwidth = 1)
